...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        q = self.to_q(x)
        context = default(context, x)   #当context=None时，contex是x，进行self-attention。contex!=None时，进行cross-attention
        k = self.to_k(context)
        v = self.to_v(context)
...
class SpatialTransformerV2d4(nn.Module):
    ...
    def forward(self, x, context=None, lr_prompt=None, seg_prompt_emb=None, seg_img_emb=None ):
        ...
        for i, block in enumerate(self.transformer_blocks):
            # [4, 4096, 320]
            x = block(x, context=context, seg_prompt_emb=seg_prompt_emb, seg_img_emb=seg_img_emb)
        if self.use_linear:
            x = self.proj_out(x)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if not self.use_linear:
